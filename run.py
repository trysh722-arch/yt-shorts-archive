...
KEEP = ["01_검색상단.jpg", "02_선반_접힘.jpg", "03_선반_전체.jpg"]
...
def raw_url(rel_path):
    ...
    enc = "/".join(quote(p) for p in Path(rel_path).parts)
    return f"https://raw.githubusercontent.com/{REPO}/{BRANCH}/{enc}"
...
def push_images(msg):
    """archive/ 를 커밋·푸시. 올릴 게 없으면 조용히 넘어간다."""
...
def main():
    ...
    for keyword, row in targets:
        try:
            ...
            failed.append((keyword, row, "실패: 선반 못 찾음"))
            print(f"[{keyword}] 선반 못 찾음")
            continue
        except Exception as e:
            failed.append((keyword, row, f"실패: {type(e).__name__}"))
            traceback.print_exc()
            continue
        done.append((keyword, row, result, archive(result, keyword)))
